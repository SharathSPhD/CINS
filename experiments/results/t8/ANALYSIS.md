...
**Deviation 7 (review-added): Bernstein level n=16 not run.** STATS_PROTOCOL §4
pre-registers n ∈ {4,6,8,10,12,16}; the executed matrix stops at n=12 (runtime
scoping at cell generation). The n-sweep figure's n=16 point belongs to the T2
Gram-conditioning CONTEXT series only, not to any Newton-solve ablation.

**Deviation 8 (review-added): LE-treatment levels Kulfan-LEM and Masters-unwrapped
not run.** Only {none, prescribed} executed; LEM is not wired into the Newton free
vector (an ADR-scoped implementation, see the T8 infra notes) and Masters-unwrapped
was never implemented. The LE ablation is therefore a 2-level contrast, not the
pre-registered 4-level factor.

**Deviation 9 (review-added): Holm-Bonferroni (§5) and exclusion rules (§6) —
explicit applicability statement.** No p-values exist anywhere in this analysis
(n=1 point values throughout), so the Holm-Bonferroni commitment is vacuously
not-applicable. The §6 exclusion-rule machinery applies to panel runs; see the
H1 panel addendum for its first real invocation (direct-solve non-convergence).

**Deviation 10 (review-added): init ablation level substitution.** The protocol's
"baseline-airfoil guess vs random" levels were narrowed to a ±5% perturbation of
A* (`n_perturb_frac=0.05`) at cell generation; documented in the cell YAML but
restated here so this document's accounting is self-contained.
...
regenerable from `experiments/results/t8/*/result.json` + `diagnostics.json`, plus `experiments/results/t2_gram_conditioning.json` (T2 context series in the n-sweep figure — external to the T8 tree, named per review) — no
