...
The mechanism on this side is the simpler of the two. Under the prescribed-LE
treatment (FM-3, \S\ref{sec:fm}), $A_{u,0}$ and $A_{l,0}$ are given rather
than solved. A target row placed inside the prescribed region therefore
constrains pressure over a piece of surface that cannot move, and contributes
almost nothing about the coefficients that remain free.
Figure~\ref{fig:le-identifiability}(a) shows this directly: the norm of
$\partial C_p/\partial A_{\text{free}}$ collapses over the prescribed region.
The consequence is the one already familiar from
Table~\ref{tab:identifiability}, reached from the opposite direction: the
extended system stays formally square and full-rank while becoming
numerically near-dependent, and Newton converges to a root that is not the
generating geometry.
...
\begin{table}[h]
\centering
\caption{Target-station placement under a prescribed leading edge, measured on
a viscous self-consistency target from an identical perturbed start. Only the
candidate set differs between the two columns. The recovered surface offset is
quoted in millichords, against the 1 millichord (0.1 percent chord) that
\S\ref{sec:formulation}'s fit gate allows a CST representation itself.}
\label{tab:le-identifiability}
\footnotesize
\begin{tabularx}{\textwidth}{@{}L c c@{}}
\toprule
Quantity & LE stations included & LE stations excluded \\
\midrule
Submap condition            & not recorded          & $19.9$ \\
Newton iterations           & 14                    & \textbf{8} \\
Final residual              & $7.19\times10^{-11}$  & $5.06\times10^{-11}$ \\
$\norm{A-A^*}_\infty$       & $3.9\times10^{-2}$    & $\mathbf{5.4\times10^{-4}}$ \\
Max surface offset (mc)     & $0.863$               & $\mathbf{0.017}$ \\
\bottomrule
\end{tabularx}
\end{table}

Both columns converge, and both report a residual at the solver floor. The
recovered geometry differs by a factor of fifty. This reinforces rather than
merely repeats \S\ref{sec:ablations}: there, the degeneracy came from
discarding between-station information across the whole chord; here it comes
from spending equations on a region that has been removed from the design
space. In both cases the residual norm is silent about it, and only a
comparison against the true coefficients or the true surface reveals the
difference.
...
(a)~the sensitivity of surface pressure to the free coefficients collapses
over the prescribed region, shaded; (b)~the stations chosen by unrestricted
